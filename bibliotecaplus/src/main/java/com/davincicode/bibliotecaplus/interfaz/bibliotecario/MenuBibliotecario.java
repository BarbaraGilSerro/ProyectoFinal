...
public class MenuBibliotecario {
        ...
        switch (funcionSeleccionada){
           ...
           //     break;
            case 0:
                ...
                break;
            case 1:
                mostrarTodosLosAutores();//Babsy deberia ser todos los libros, pero como no hay nada y necesito ver si funciona lo hice con los autores hasta que este completado BDLibros y LibroRepository...
                break;
            case 2:
    ...
    private void mostrarTodosLosAutores(){
        List<Autor> listaAutores = buscarTodosLosAutores(); //llama al metodo que busca todos los autores
        mostrarDatosTodosLosAutores (listaAutores);
    }
    private List<Autor> buscarTodosLosAutores(){
        AutorServicio autorServicio = new AutorServicio();  //crea un objeto de tipo autoservicio para acceder al metodo findAllAutores
        return autorServicio.findAllAutores(); //llama al metodo para buscar todos los autores       
    }
    private void mostrarDatosTodosLosAutores(List<Autor> listaAutores){
        StringBuilder todosLosAutores = new StringBuilder("Autores encontrados:\n");
                for (Autor autor :listaAutores){
                    todosLosAutores.append("Nombre: ").append(autor.getNombre())
                    .append(", Apellido: ").append(autor.getApellido())
                    .append(", Nacionalidad: ").append(autor.getNacionalidad()).append("\n");        
                }
                JOptionPane.showMessageDialog(null, todosLosAutores.toString(), "Restultados de Busqueda", JOptionPane.INFORMATION_MESSAGE);
    }
